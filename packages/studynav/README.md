...
- On supported articles, select the words you need; the selection action bar offers highlight colors, Add note, Copy, and Link without placing hover controls over the article. Image downloads are opt-in under **Feature settings** and use a labeled download button beside eligible images. The popup also has a shortcut to the dedicated Google image search for JW.org.
