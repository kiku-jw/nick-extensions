...
2. Select text inside one paragraph or verse to highlight it in one of four colors, optionally add a private note and tags, and review it through **Study library**. Save the current page, paragraph, or verse under **Saved places**. Notes, saved places, and watch progress stay in local extension storage; JSON export/import provides a manual backup.
3. Use **Copy citation**, **Show QR**, or **Open official JW link** for safe page-derived sharing actions. QR generation is local and the displayed target URL remains visible.
...
5. On articles, hover or keyboard-focus supported text for Copy and Link. Image downloads are off by default; enable **Download article images** to add a compact icon button. On media pages, use time-link, second-display, transcript, keyboard, and explicit resume controls.
